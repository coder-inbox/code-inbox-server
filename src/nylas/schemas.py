...
class SuccessUrlSchema(BaseModel):
    ...
    email_address: EmailStr = Field(
        ..., description="Sender's email address", example="sender@example.com"
    )
...
class CodeTokenSchema(BaseModel):
    ...
    token: str = Field(
        ...
        example="XIHkPTgVfcirb6V7C8U8zNzc9mtdxr",
    )


class MailRecipient(BaseModel):
    name: str = Field(
        ..., description="Recipient's name", example="Receiver name"
    )
...
class SendEmailSchema(BaseModel):
    ...
    cc: Optional[str] = Field(
        ..., description="CC email address", example="cc@example.com"
    )
    bcc: Optional[str] = Field(
        ..., description="BCC email address", example="bcc@example.com"
    )
    subject: str = Field(
        ..., description="Email subject", example="Hello, World!"
    )
    message: str = Field(
        ...,
